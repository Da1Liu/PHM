  ...
  machines: [
    {
      ...
      systems: {
        ...
        hydraulic: { mode: "status_only", light: "l1", message: "hydraulic status normal" },
      },
    },
    {
...
const LIGHT_CN = { green: "normal", yellow: "watch", red: "alarm", slate: "building", l1: "status" };
...
function machineHeadline(m) {
  ...
  if (bld) return { num: Math.round(100 * bld.n / bld.N) + "%", sub: "building" };
...
function tabOverview(m, host) {
  ...
  const ack = host.querySelector("[data-ack]"); if (ack) ack.onclick = () => toast("alarm acknowledged");
}
...
function tabRun(m, host) {
  ...
  const both = r.opcua && r.ni, none = !r.opcua && !r.ni;
  const pill = (label, on) => `<span class="pill ${on ? "on" : ""}">${label} - ${on ? "running" : "stopped"}</span>`;
  // 主操作随状态变: 运行中主操作是“停止”(quiet), 全停时才给填充“启动”
  const unified = none
    ? wbtn("Start all", "btn--primary", "", "start-all")
    : wbtn("Stop all", "btn--danger-quiet", "", "stop-all") + (both ? "" : wbtn("Start all", "btn--secondary", "", "start-all"));
  host.innerHTML = `<div class="card">
    <h3>统一采集控制 · ${esc(m.id)}</h3>
    <div class="statline">${pill("OPC UA", r.opcua)}${pill("NI 振动", r.ni)}${r.ni ? '<span class="tiny mono">124 行 · 25.6k sps</span>' : ""}</div>
    <p class="sub">一次性同时启停 OPC UA + NI, 避免人工分别启动产生时间差 (关系到跨源数据对齐)。</p>
    <div class="row" style="margin-top:10px">${unified}<span class="sub" style="margin-left:6px">当前: ${both ? "全部运行" : none ? "全部停止" : "部分运行"}</span></div>
    <div class="row" style="margin-top:12px"><span class="sub">单独</span>
      ${wbtn("OPC UA " + (r.opcua ? "Stop" : "Start"), r.opcua ? "btn--danger-quiet btn--sm" : "btn--ghost btn--sm", "", "tg-opcua")}
      ${wbtn("NI " + (r.ni ? "Stop" : "Start"), r.ni ? "btn--danger-quiet btn--sm" : "btn--ghost btn--sm", "", "tg-ni")}</div>
  </div>
  <div class="card">
    <h3>波形查看 / 抓取</h3>
    <div class="cfg-row"><span>通道</span>
      <select><option>ch_sx 主轴X振动 · 高频</option><option>ch_sy 主轴Y振动 · 高频</option><option>ch_sz 主轴Z振动 · 高频</option></select></div>
    <div class="row">
      <button class="btn btn--secondary">查看波形</button>
      ${wbtn("抓取原始波形", "btn--ghost", "", "capture")}
    </div>
    <div class="note">默认列出该机床高频(振动)通道 (按 NI 型号登记)。查看走真实波形 (无块回退占位); 抓取通知采集器抓所选通道。静态原型不发请求。</div>
  </div>`;
  host.querySelectorAll("[data-act]").forEach(b => b.onclick = async () => {
    if (document.body.dataset.role === "operator") return; // 操作工由全局门控拦截
    const a = b.dataset.act;
    if (NET.online) {  // 写真实 acq_config.control, 回读 collector-status 刷新
      const map = { "start-all": ["all", "start"], "stop-all": ["all", "stop"],
        "tg-opcua": ["opcua", r.opcua ? "stop" : "start"], "tg-ni": ["ni", r.ni ? "stop" : "start"], "capture": ["ni", "capture"] };
      const [target, action] = map[a];
      await postJSON(`/api/machine/${m.id}/control`, { target, action });
      const cs = await getJSON(`/api/machine/${m.id}/collector-status`);
      RUN[m.id] = { opcua: !!(cs.opcua && cs.opcua.run), ni: !!(cs.ni && cs.ni.run) };
      STATUS[m.id] = statusFromApi(cs);
      if (a === "start-all") toast("start requested"); else if (a === "stop-all") toast("stop requested"); else if (a === "capture") toast("capture requested");
      return render();
    }
    if (a === "start-all") { r.opcua = r.ni = true; toast("start requested"); }
    else if (a === "stop-all") { r.opcua = r.ni = false; toast("stop requested"); }
    else if (a === "tg-opcua") r.opcua = !r.opcua;
    else if (a === "tg-ni") r.ni = !r.ni;
    else if (a === "capture") return toast("capture requested");
    render();
  });
}
...
/* ---------- 设置 · 建档 (机床管理 / 信号映射 / 采集配置 / 同步状态) ---------- */
const SET_TABS = [["machines", "Machines"], ["mapping", "Signals"], ["acq", "Acquisition"], ["status", "Status"]];
function gatedBlocked() { // 行内 read 控件触发写时的兜底 (data-write 按钮已由全局门控拦截)
  if (document.body.dataset.role === "operator") { toast("engineer role required"); return true; }
  return false;
}
function renderSettings(app) {
  ...
  app.innerHTML = `
    ...
    <div class="row between wrap" style="margin-bottom:var(--s3)">
      <h1 class="page-title">设置 · 建档</h1><span class="chip slate">写操作需工程师权限</span></div>
  ...
  ({ machines: setMachines, mapping: setMapping, acq: setAcq, status: setStatus }[tab])($("#setbody"));
}

function setMachines(host) {
  host.innerHTML = `<div class="card">
    <table class="tbl"><thead><tr><th>机床 SN</th><th>数控系统</th><th>epoch</th><th>系统</th><th></th></tr></thead>
    <tbody>${DATA.machines.map(m => `<tr>
      ...
      <td style="white-space:nowrap;text-align:right"><button class="lk" data-view2="${esc(m.id)}">查看</button>
        ${wbtn("删除", "btn--danger-quiet btn--sm", "", "del-" + m.id)}</td></tr>`).join("")}</tbody></table>
    <p class="sub" style="margin-top:8px">共 ${DATA.machines.length} 台 · 每台独立 signal 维表 + acq_config + epoch, 协议/地址可各不相同。</p>
  </div>
  <div class="card"><h3>接入新机床</h3>
    <label class="cfg-row"><span>机床 SN</span><input id="nm_id" placeholder="如 CNC_05"></label>
    <label class="cfg-row"><span>数控系统</span><input id="nm_cnc" placeholder="siemens_840d / huazhong / fanuc …"></label>
    <div class="row">${wbtn("接入", "btn--primary", "", "add-machine")}</div>
    <div class="note">接入后: ① 采集配置设采样 连接 ② 信号映射登记通道(probe) ③ 部署边缘网关。新机床即出现在机群与大屏。静态原型不写库。</div>
  </div>`;
  host.querySelectorAll("[data-view2]").forEach(b => b.onclick = () => openMachine(b.dataset.view2));
  host.querySelectorAll("[data-act]").forEach(b => b.onclick = async () => {
    if (document.body.dataset.role === "operator") return;
    const a = b.dataset.act;
    if (a === "add-machine") {
      const id = $("#nm_id").value.trim(); if (!id) return toast("请填机床 SN");
      if (NET.online) {
        const res = await postJSON("/api/machines", { machine_id: id, cnc_system: $("#nm_cnc").value.trim() });
        if (!res.ok) return toast("failed: " + (res.error || "")); toast(res.msg || "created");
        await reloadAll(); render();
      } else toast("created " + id);
      return;
    }
    if (a.indexOf("del-") === 0) {
      const id = a.slice(4); if (!confirm(`删除机床 ${id} 及其全部数据? 不可恢复`)) return;
      if (NET.online) {
        const res = await postJSON(`/api/machines/${encodeURIComponent(id)}`, {}, "DELETE");
        if (!res.ok) return toast("delete failed: " + (res.error || "")); toast("deleted " + id);
        await reloadAll(); if (!DATA.machines.find(m => m.id === S.machine)) S.machine = (DATA.machines[0] || {}).id; render();
      } else toast("deleted " + id);
    }
  });
}
...
function setMapping(host) {
  ...
  const body = rows.length ? rows.map(mapRow).join("")
    : `<tr><td colspan="8" class="sub">该机床尚无信号映射。点「＋新增信号」逐条登记, 或「从机床克隆」复制一台已配好的再逐条改地址。</td></tr>`;
  host.innerHTML = `<div class="card">
    <div class="map-toolbar">
      ${wbtn("＋ 新增信号", "btn--secondary btn--sm", "", "map-new")}
      ${wbtn("Clone", "btn--ghost btn--sm", "", "map-clone")}
      ${wbtn("导入", "btn--ghost btn--sm", "", "map-import")}
      <button class="btn btn--ghost btn--sm" id="mapExport">导出</button>
      <span class="sub">${esc(S.machine)} · ${rows.length} 信号</span></div>
    <div id="mapform"></div>
    <table class="tbl"><thead><tr><th>编码</th><th>名称</th><th>系统</th><th>类型</th><th>协议</th><th>角色</th><th>订阅地址</th><th></th></tr></thead>
      <tbody>${body}</tbody></table>
    <p class="sub" style="margin-top:10px">多协议可扩展: 新机型 新协议只加行不改表。地址 probe 实测需 live 采集器 (占位)。</p></div>`;
  if (MAPED.editing != null) renderMapForm();
  $("#mapExport").onclick = async () => {
    if (NET.online) {
      const r = await getJSON(`/api/machine/${S.machine}/signals/export`);
      ...
      aEl.download = `signals_${S.machine}.json`; aEl.click(); URL.revokeObjectURL(aEl.href);
      toast(`exported ${(r.signals || []).length} signals`);
    } else toast(`exported ${rows.length} signals`);
  };
  host.querySelectorAll("[data-act]").forEach(b => b.onclick = () => mapAction(b.dataset.act));
  host.querySelectorAll("[data-edit]").forEach(b => b.onclick = () => { if (gatedBlocked()) return; MAPED.editing = +b.dataset.edit; renderMapForm(); });
  host.querySelectorAll("[data-del]").forEach(b => b.onclick = async () => {
    if (gatedBlocked()) return; const id = +b.dataset.del;
    if (NET.online) {
      const r = await postJSON(`/api/machine/${S.machine}/signals/${id}`, {}, "DELETE");
      if (!r.ok) return toast("delete failed: " + (r.error || "")); toast("deleted"); await reloadSignals(S.machine); render();
    } else toast("deleted signal");
  });
}
function mapRow(s) {
  let role = "";
  ...
  return `<tr><td class="mono">${esc(s.code)}</td><td>${esc(s.name)}</td><td>${SYS_CN[s.system] || ""}</td>
    ...
    <td class="tiny mono" style="word-break:break-all">${esc(s.addr || "")}</td>
    <td style="white-space:nowrap;text-align:right"><button class="lk" data-edit="${s.id}">编辑</button>
      <button class="lk lk--danger" data-del="${s.id}">删除</button></td></tr>`;
}
function mapAction(a) { // map-new/clone/import 为 data-write 按钮: 操作工已被全局门控拦截, 此处仅工程师可达
  if (a === "map-new") { MAPED.editing = "new"; renderMapForm(); }
  else if (a === "map-clone") mapClone();
  else if (a === "map-import") mapImport();
}
function mapClone() {
  if (!NET.online) return toast("从机床克隆，选源机床→复制信号集→清空地址逐台改 (占位)");
  const others = DATA.machines.filter(m => m.id !== S.machine).map(m => m.id);
  if (!others.length) return toast("无其他机床可克隆");
  const from = prompt("从哪台机床克隆？输入 SN:\n可选: " + others.join(", "));
  if (!from) return;
  postJSON(`/api/machine/${S.machine}/signals/clone`, { from: from.trim(), keep_addr: false }).then(async r => {
    if (!r.ok) return toast("clone failed: " + (r.error || "")); toast(`cloned ${r.count} signals`); await reloadSignals(S.machine); render();
  });
}
function mapImport() {
  if (!NET.online) return toast("导入 JSON 信号集 · 替换/合并 (占位)");
  const inp = document.createElement("input"); inp.type = "file"; inp.accept = ".json,application/json";
  inp.onchange = async () => {
    const f = inp.files[0]; if (!f) return;
    let arr; try { arr = JSON.parse(await f.text()); } catch (e) { return toast("JSON parse failed"); }
    if (!Array.isArray(arr)) arr = (arr && Array.isArray(arr.signals)) ? arr.signals : null;
    if (!arr) return toast("content must be a signal array");
    const replace = confirm(`导入 ${arr.length} 信号\n确定=替换(先清空)  取消=合并`);
    const r = await postJSON(`/api/machine/${S.machine}/signals/import`, { signals: arr, mode: replace ? "replace" : "merge" });
    if (!r.ok) return toast("import failed: " + (r.error || "")); toast(`imported ${r.count} signals (${r.mode})`); await reloadSignals(S.machine); render();
  };
  inp.click();
}
function renderMapForm() {
  const box = $("#mapform"); if (!box) return;
  const isNew = MAPED.editing === "new";
  const s = isNew ? {} : (SIGNALS[S.machine] || []).find(x => x.id === MAPED.editing) || {};
  const PROTOS = ["opcua", "nclink", "ni_daq"], KINDS = ["vibration", "temperature", "pressure", "current", "speed", "position", "bool"];
  const sysP = [["", "(无)"], ["spindle", "主轴"], ["feed", "进给"], ["hydraulic", "液压"]];
  const tmpP = [["", "none"], ["coupled", "coupled"], ["confound", "confound"]];
  const fld = (id, l, v) => `<label class="cfg-row"><span>${l}</span><input id="${id}" value="${esc(v || "")}"></label>`;
  const sel = (id, l, opts, v) => `<label class="cfg-row"><span>${l}</span><select id="${id}">${opts.map(o => `<option ${o === v ? "selected" : ""}>${o}</option>`).join("")}</select></label>`;
  const selP = (id, l, pairs, v) => `<label class="cfg-row"><span>${l}</span><select id="${id}">${pairs.map(([vv, t]) => `<option value="${vv}" ${vv === (v || "") ? "selected" : ""}>${t}</option>`).join("")}</select></label>`;
  box.innerHTML = `<div class="map-form"><h3>${isNew ? "新增信号" : "编辑 · " + esc(s.code)}</h3>
    <div class="mf-grid">
      ${fld("mf_code", "编码 code*", s.code)}${fld("mf_name", "名称", s.name)}${fld("mf_unit", "单位", s.unit)}
      ${sel("mf_proto", "协议", PROTOS, s.protocol || "opcua")}${sel("mf_kind", "类型 kind*", KINDS, s.kind || "vibration")}
      ${selP("mf_sys", "System", sysP, s.system)}${selP("mf_temp", "Temp role", tmpP, s.temp_role)}
      <label class="cfg-row"><span>Regime</span><input type="checkbox" id="mf_regime" ${s.regime ? "checked" : ""}></label>
      <label class="cfg-row"><span>高频(振动)</span><input type="checkbox" id="mf_hf" ${s.high_freq ? "checked" : ""}></label>
    </div>
    <label class="cfg-row"><span>订阅地址</span><input id="mf_addr" value="${esc(s.addr || "")}" style="max-width:none"></label>
    <div class="row" style="margin-top:8px">${wbtn(isNew ? "Create" : "Save", "btn--primary", "", "map-save")}
      <button class="btn btn--ghost" id="mapCancel">取消</button></div></div>`;
  box.querySelector("[data-act]").onclick = async () => {
    if (gatedBlocked()) return;
    if (NET.online) {
      const body = { code: $("#mf_code").value.trim(), display_name: $("#mf_name").value.trim(), unit: $("#mf_unit").value.trim(),
        protocol: $("#mf_proto").value, signal_kind: $("#mf_kind").value, phm_system: $("#mf_sys").value,
        temp_role: $("#mf_temp").value, regime_role: $("#mf_regime").checked, is_high_freq: $("#mf_hf").checked,
        source_addr: $("#mf_addr").value.trim() };
      if (!body.code) return toast("编码 code 必填");
      const url = isNew ? `/api/machine/${S.machine}/signals` : `/api/machine/${S.machine}/signals/${MAPED.editing}`;
      const r = await postJSON(url, body, isNew ? "POST" : "PUT");
      if (!r.ok) return toast("failed: " + (r.error || "")); toast(isNew ? "created" : "saved");
      MAPED.editing = null; await reloadSignals(S.machine); render(); return;
    }
    toast(isNew ? "created" : "saved"); MAPED.editing = null; render();
  };
  $("#mapCancel").onclick = () => { MAPED.editing = null; render(); };
}

function setAcq(host) {
  const a = ACQ[S.machine] || ACQ_DEF;
  const f = (l, v, u = "") => `<label class="cfg-row"><span>${l}</span><input value="${esc(v ?? "")}"><em>${u}</em></label>`;
  const sel = (l, v, opts) => `<label class="cfg-row"><span>${l}</span><select>${opts.map(o => `<option ${o === v ? "selected" : ""}>${o}</option>`).join("")}</select></label>`;
  const chk = (l, on, u = "") => `<label class="cfg-row"><span>${l}</span><input type="checkbox" ${on ? "checked" : ""}><em>${u}</em></label>`;
  host.innerHTML = `<div class="card"><h3>NI 高频振动采集</h3>
    ${sel("Source", a.ni.source, ["simulated", "nidaq"])}${f("Rate", a.ni.rate, "Hz")}
    ${f("Samples/channel", a.ni.samplesPerChannel, "samples")}${f("Input buffer", a.ni.bufferSize, "samples")}
    ${f("Table prefix", a.ni.tableBase)}${f("Feature window", a.ni.featureWin, "0=1s")}
    ${chk("Event trigger", a.ni.eventEnabled, "raw capture")}${f("Event RMS", a.ni.eventThr, "g")}
    <h4 style="margin:14px 0 6px;color:var(--text-2)">通道与灵敏度</h4>
    <table class="tbl"><thead><tr><th>Physical channel</th><th>Sensitivity (mV/g)</th></tr></thead>
      <tbody>${a.ni.channels.map(c => `<tr><td><input value="${esc(c.pc)}"></td><td><input value="${esc(c.sens)}"></td></tr>`).join("")}</tbody></table>
  </div>
  <div class="card"><h3>OPC UA 状态量</h3>
    ${chk("Enabled", a.opcua.enabled)}${sel("Profile", a.opcua.profile, ["kepserver", "machine"])}
    ${f("Endpoint", a.opcua.endpoint)}${chk("匿名", a.opcua.anonymous)}
    ${f("Username", a.opcua.username)}${f("Password", a.opcua.password)}${f("Poll interval", a.opcua.pollMs, "ms")}
  </div>
  <div class="card"><h3>NC-Link (后续台份)</h3>
    ${f("Host", a.nclink.host)}${f("Port", a.nclink.port)}${f("SN", a.nclink.sn)}</div>
  <div class="row" style="margin-top:var(--s4)">${wbtn("保存配置", "btn--primary", "", "acq-save")}<a class="btn btn--secondary" href="${esc(a.edge?.baseUrl || "http://localhost:4000")}" target="_blank" rel="noreferrer">打开采集工作台</a></div>
  <div class="note">结构对齐采集线 app_config。保存写入 phm_v2.acq_config; 边缘采集器读它启动采集。静态原型不写库。</div>`;
  host.querySelectorAll("[data-act]").forEach(b => b.onclick = async () => {
    if (document.body.dataset.role === "operator") return;
    if (NET.online) { const r = await postJSON(`/api/machine/${S.machine}/acq-config`, acqToApi(a), "PUT");
      return toast(r.ok ? "config saved" : "save failed: " + (r.error || "")); }
    toast("config saved");
  });
}
...
function setStatus(host) {
  ...
  host.innerHTML = `<div class="card"><h3>边缘 · 同步状态<span class="badge">只读</span></h3>
    <table class="tbl"><tbody>
      ...
      <tr><th>OPC UA 采集器</th><td>${esc(s.opcua)}</td></tr>
      <tr><th>NI 采集器</th><td>${esc(s.ni)} · 心跳 <span class="mono">${esc(s.heartbeat)}</span></td></tr>
    </tbody></table>
...
function initTopbar() {
  ...
  $("#roleBtn").onclick = () => {
    ...
    $("#roleBtn").textContent = op ? "Role: engineer" : "Role: operator";
    toast(op ? "engineer mode" : "operator mode");
...
function acqToApi(a) {  // 反向映射回后端 shape (保存时)
  return {
    acquisition: { source: a.ni.source, rate: +a.ni.rate, samplesPerChannel: +a.ni.samplesPerChannel,
      inputBufferSize: +a.ni.bufferSize, tableBaseName: a.ni.tableBase, featureWindowSamples: +a.ni.featureWin,
      eventEnabled: !!a.ni.eventEnabled, eventRmsThresholdG: +a.ni.eventThr,
      channels: a.ni.channels.map(c => ({ physicalChannel: c.pc, sensitivityMvPerG: +c.sens })) },
    opcua: { enabled: !!a.opcua.enabled, profile: a.opcua.profile, endpoint: a.opcua.endpoint, anonymous: !!a.opcua.anonymous,
      username: a.opcua.username, password: a.opcua.password, pollIntervalMs: +a.opcua.pollMs },
    nclink: { host: a.nclink.host, port: +a.nclink.port, sn: a.nclink.sn },
    edge: a.edge || {},
  };
}
