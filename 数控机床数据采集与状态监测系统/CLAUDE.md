...
[任意主机] api (Node + Express)          ▼
   ...
   ├─ 读 API（看板/趋势/波形）<──────────┘
   └─ 配置 API + 采集开关 API（写 app_config / collector_control）
...
- `src/configStore.js`：`app_config`(JSONB) 读写 + DEFAULTS（采集模块共享配置面）。
...
- `src/opcua/`：`poller.js`（轮询落库 + start/stop/restart/getStatus）、`config.js`（两套 profile）、`schema.js`（建 _OPCUA_* 表）、`transform.js`（类型转换）。
...
- `config.html`：采集配置页（替代旧 Form1）——采样率/通道/灵敏度/表名/落盘参数 + OPC UA 连接，读写 `/api/config`。
- ECharts 走 CDN。
...
- OPC UA: Node API 每 1s reconcile `control.opcua_run` 与 OPC UA 配置签名, 自动启动/停止/重启 poller。
...
- 信号目录: WebDashboard `/api/opcua/catalog` 与 `/api/signals/catalog` 从 `phm_v2.signal` 生成; 缺登记时显示“未建档”, 不把硬编码 catalog 当集成默认。
